...
#define LOCTEXT_NAMESPACE "SOFSMEdge"

void SEdNode_OFSMEdge::Construct(const FArguments& InArgs, UEdNode_OFSMEdge* InNode)
{
	this->GraphNode = InNode;
	this->UpdateGraphNode();
}

bool SEdNode_OFSMEdge::RequiresSecondPassLayout() const
{
	return true;
}

void SEdNode_OFSMEdge::PerformSecondPassLayout(const TMap< UObject*, TSharedRef<SNode> >& NodeToWidgetLookup) const
{
	UEdNode_OFSMEdge* EdgeNode = CastChecked<UEdNode_OFSMEdge>(GraphNode);
...
void SEdNode_OFSMEdge::UpdateGraphNode()
{
	InputPins.Empty();
	OutputPins.Empty();
	...
	this->GetOrAddSlot( ENodeZone::Center )
		...
		[
			...
			[
				...
				.ColorAndOpacity( this, &SEdNode_OFSMEdge::GetEdgeColor )
			]
			+SOverlay::Slot()
			[
				SNew(SImage)
...
void SEdNode_OFSMEdge::PositionBetweenTwoNodesWithOffset(const FGeometry& StartGeom, const FGeometry& EndGeom, int32 NodeIndex, int32 MaxNodes) const
{
...
FSlateColor SEdNode_OFSMEdge::GetEdgeColor() const
{
