...
{
	EdGraph = Cast<UEdGraph_OFSM>(_EdGraph);
	check(EdGraph != nullptr);

	EdGraph->RebuildOFSM();
	Graph = EdGraph->GetOFSM();
	check(Graph != nullptr);
	...
	{
		UOFSMNode* RootNode = Graph->RootNodes[i];
...
void UTreeLayoutStrategy::InitPass(UOFSMNode* RootNode, const FVector2D& Anchor)
{
	UEdNode_OFSMNode* EdNode_RootNode = EdGraph->NodeMap[RootNode];
	...
	{
		UOFSMNode* Child = RootNode->ChildrenNodes[i];
		UEdNode_OFSMNode* EdNode_ChildNode = EdGraph->NodeMap[Child];
		if (i > 0)
		{
			UOFSMNode* PreChild = RootNode->ChildrenNodes[i - 1];
			UEdNode_OFSMNode* EdNode_PreChildNode = EdGraph->NodeMap[PreChild];
...
bool UTreeLayoutStrategy::ResolveConflictPass(UOFSMNode* Node)
{
	bool HasConflict = false;
	for (int32 i = 0; i < Node->ChildrenNodes.Num(); ++i)
	{
		UOFSMNode* Child = Node->ChildrenNodes[i];
	...
	{
		UOFSMNode* ParentNode = Node->ParentNodes[i];
		for (int32 j = 0; j < ParentNode->ChildrenNodes.Num(); ++j)
		{
			UOFSMNode* LeftSibling = ParentNode->ChildrenNodes[j];
			if (LeftSibling == Node)
				break;
...
bool UTreeLayoutStrategy::ResolveConflict(UOFSMNode* LRoot, UOFSMNode* RRoot)
{
	TArray<UEdNode_OFSMNode*> RightContour, LeftContour;
	...
	if (MaxOverlapDistance > 0)
	{
		...
		TArray<UOFSMNode*> ParentNodes = RRoot->ParentNodes;
		TArray<UOFSMNode*> NextParentNodes;
...
void UTreeLayoutStrategy::GetLeftContour(UOFSMNode* RootNode, int32 Level, TArray<UEdNode_OFSMNode*>& Contour)
{
	UEdNode_OFSMNode* EdNode_Node = EdGraph->NodeMap[RootNode];
...
void UTreeLayoutStrategy::GetRightContour(UOFSMNode* RootNode, int32 Level, TArray<UEdNode_OFSMNode*>& Contour)
{
	UEdNode_OFSMNode* EdNode_Node = EdGraph->NodeMap[RootNode];
	if (Level >= Contour.Num())
	{
...
void UTreeLayoutStrategy::ShiftSubTree(UOFSMNode* RootNode, const FVector2D& Offset)
{
	UEdNode_OFSMNode* EdNode_Node = EdGraph->NodeMap[RootNode];
...
void UTreeLayoutStrategy::UpdateParentNodePosition(UOFSMNode* ParentNode)
{
	UEdNode_OFSMNode* EdNode_ParentNode = EdGraph->NodeMap[ParentNode];
	if (ParentNode->ChildrenNodes.Num() % 2 == 0)
	{
		UEdNode_OFSMNode* FirstChild = EdGraph->NodeMap[ParentNode->ChildrenNodes[0]];
		UEdNode_OFSMNode* LastChild = EdGraph->NodeMap[ParentNode->ChildrenNodes.Last()];
	...
	{
		UEdNode_OFSMNode* MidChild = EdGraph->NodeMap[ParentNode->ChildrenNodes[ParentNode->ChildrenNodes.Num() / 2]];
